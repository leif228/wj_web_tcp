...
public class InBusinessHandler extends ChannelInboundHandlerAdapter {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        if (msg instanceof byte []){
            log.debug("解码后的字节码："+new String((byte[]) msg,"UTF-8"));
            try {
                ...
                if (objectContainer instanceof DTObject){
                    ...
                    if (data.getClassName()!=null&&data.getObject().length>0){
                        ...
                        log.debug("收到实体对象："+object);
                        //测试
                        this.sendToClient(ctx);
                    }
                }
            }catch (Exception e){
                log.debug("对象反序列化出现问题："+e);
            }
    ...
    private void sendToClient(ChannelHandlerContext ctx) {
        try {
            ...
            ctx.channel().writeAndFlush(tcpProtocol);
        }catch (Exception e){
            log.debug("生成对象出现问题："+e);
        }
...
//    @Override
//    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
//        log.debug("InBusinessHandler# # 给客户端 发送数据 ... :" + ctx.channel() + "==>数据：" + msg.toString());
